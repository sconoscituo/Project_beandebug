    ...
    <div style={{ paddingTop: '70px' }}>
      ...
      <section className="section">
        <div className="section-header">
          ...
          <Link to="/recipes" className="section-link">전체보기 →</Link>
        </div>
      ...
      <section className="featured-section">
        <div className="featured-content">
          <div className="featured-text">
            ...
            <h2 className="featured-title">에티오피아<br />예가체프 코체레</h2>
            <p className="featured-description">
              꽃향과 시트러스, 그리고 홍차를 연상시키는 우아한 풍미.<br />
              이번 달 추천 원두로 새로운 커피 경험을 시작하세요.
            </p>
            <Link to="/beans" className="btn-primary">원두 살펴보기 →</Link>
          </div>
          <div className="featured-image">
            <img src="https://images.unsplash.com/photo-1559525839-b184a4d698c7?w=600" alt="Featured Bean" />
          </div>
        </div>
      </section>
      ...
      <section className="section">
        ...
        <div className="card-grid">
          {latestArticles.map((article, index) => (
            <Link key={article.id} to={`/articles/${article.id}`} className="card"
              style={{
                opacity: isLoaded ? 1 : 0,
                transform: isLoaded ? 'translateY(0)' : 'translateY(30px)',
                transition: `all 0.6s ease ${index * 0.1}s`
      ...
      <section style={{ background: 'linear-gradient(135deg, #f5f1eb 0%, #e8e0d5 100%)', padding: '5rem 2rem', textAlign: 'center' }}>
        ...
        <p style={{ color: '#666', fontSize: '1.1rem', marginBottom: '2rem', maxWidth: '600px', margin: '0 auto 2rem' }}>
      ...
      <footer className="footer">
        ...
        <div className="footer-bottom">© 2026 Bean Debug. All rights reserved.</div>
      </footer>
    </div>
  );
};
